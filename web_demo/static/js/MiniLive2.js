...
if (tag_ios) {
    ...
    if (match && match[1]) {
        ...
        if (!tag_ios17) {
            alert("iOS系統目前不支援iOS17以下版本，請升級後再試");
        }
    } else {
        // 無法獲取版本號的情況
        alert("無法檢測您的iOS版本，請確保使用iOS17或更高版本");
    }
}

let fps_enabled = true; // 全域參數，控制是否顯示FPS
let frameTimes = []; // 用於儲存最近幾幀的時間戳
...
class VideoProcessor {
    ...
    async init(videoUrl, gzipUrl) {
        ...
        if (this.mp4box) {
            ...
            this.mp4box.flush(); // 清空緩衝區
            this.mp4box = null; // 銷毀舊實例
        }
        this.mp4box = MP4Box.createFile(); // 建立新實例
    ...
    handleReady(info) {
        ...
        this.offscreenCtx = this.offscreenCanvas.getContext('2d', { alpha: false }); // 停用 Alpha
    ...
    handleVideoFrame(videoFrame) {
        if (tag_ios17)
        {
            createImageBitmap(videoFrame).then(img => {
                ...
                const sizeInMB = (blob.size / (1024 * 1024)).toFixed(2); // 保留兩位小數
...
let asset_dir = null; // 將由配置文件決定
let isPaused = false; // 标志位，控制是否暂停处理
let avatarConfig = null; // 存储人物配置
...
function hideLoadingMessage() {
    ...
    if (startMessage) {
        ...
        console.log('Start message hidden');
    }
...
async function loadAvatarConfig() {
    try {
        ...
        // 設置默認人物
        const defaultAvatar = avatarConfig.avatars.find(a => a.id === avatarConfig.default);
        if (defaultAvatar) {
            asset_dir = defaultAvatar.path;
            console.log('使用默認人物:', asset_dir);
        }
        
        return avatarConfig;
    } catch (error) {
        console.error('載入人物配置失敗:', error);
        // 顯示錯誤警示
        console.error('❌ 人物配置載入失敗！請檢查 avatars.json 文件');
        alert('⚠️ 人物配置載入失敗！\n\n請確認：\n1. avatars.json 文件存在\n2. 文件格式正確\n3. 人物資料夾完整\n\n請參考 avatars.sample.json 範例文件');
        throw new Error('Avatar configuration failed to load');
    }
}

// 動態生成人物選擇下拉選單
function populateAvatarDropdown(config) {
    const dropdown = document.getElementById('characterDropdown');
    if (!dropdown) {
        console.warn("characterDropdown 元素未找到");
        return;
    }
    
    dropdown.innerHTML = ''; // 清空現有選項
    ...
    // 設置默認選項
    const defaultAvatar = config.avatars.find(a => a.id === config.default);
    if (defaultAvatar) {
        dropdown.value = defaultAvatar.path;
    }
}
...
if (characterDropdown) {
    characterDropdown.addEventListener('change', async function() {
        try {
            ...
            setTimeout(() => {
                hideLoadingMessage();
                console.log('Character switch completed:', asset_dir);
            }, 500);
...
async function newVideoTask() {
    try {
        // 確保配置已載入且 asset_dir 已設置
        if (!asset_dir) {
            console.log('正在載入人物配置...');
            const config = await loadAvatarConfig();
            if (config) {
                populateAvatarDropdown(config);
            }
            
            // 如果仍然沒有 asset_dir，顯示錯誤警示
            if (!asset_dir) {
                console.error('❌ 人物配置載入失敗！請檢查 avatars.json 文件');
                alert('⚠️ 人物配置載入失敗！\n\n請確認：\n1. avatars.json 文件存在\n2. 文件格式正確\n3. 人物資料夾完整\n\n請參考 avatars.sample.json 範例文件');
                throw new Error('Avatar configuration failed to load');
        ...
        setTimeout(() => {
            hideLoadingMessage();
            console.log('Initial video task completed:', asset_dir);
        }, 500);
        
    } catch (error) {
        console.error('newVideoTask failed:', error);
        hideLoadingMessage();
        throw error;
    }
}
